...
const inputPathFile = path.join(inputPath, 'AACR-Congress-1.0.0.exe');
const destinationPath = path.join(outputPath, 'AACR-Congress-1.0.0.exe');
try {
	...
	if (fs.existsSync(inputPath)) {
        ...
        // Zip
        const outputZip = path.join(outputPath, `aacr-congress-package-${Date.now()}.zip`);
