...
function createNewCSVFile() {
	...
	const adcTrackingPath = path.join(documentsPath, 'AAD-DSE');
	...
	csvFilePath = path.join(adcTrackingPath, `AAD-DSE-backup_${dateTimeString}.csv`);

	// Create the file with the headers if required
	const headers = 'event,page_id,page_path,value,timestamp\n';
...
function convertToCSV(data) {
	return data
		.map((item) => {
			...
			return [event, pageId, page_path, value, now].join(',');
		})
		.join('\n');
}
